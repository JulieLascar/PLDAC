...
# On considère uniquement les tweets avec hashtags.
tweets_cible=dict()
tweets_noncible=[]
...
pickle.dump(tweets_cible,open("tweets_cible.p","wb"))

dico_tweet = pickle.load(open("dico_tweet.p","rb"))

L_t= list(tweets_cible.keys())

# Fonction pour tester l'hypothèse : un seul candidat en hashtag --> le candidat est la cible
def appli_cible (nb) :
...
#Attention! Risque d'effacer les données! Seulement pour initialiser.
#R_cible=[]
#R_polarite=dict()
#pickle.dump(R_cible,open("R_cible.p","wb"))
#pickle.dump(R_polarite,open("R_polarite.p","wb"))
